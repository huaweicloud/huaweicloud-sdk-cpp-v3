...
bool ListLtsConfigsResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("instance_lts_configs"))) {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<InstanceLogConfig> refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
...
std::vector<InstanceLogConfig>& ListLtsConfigsResponse::getInstanceLtsConfigs()
{
    return instanceLtsConfigs_;
}

void ListLtsConfigsResponse::setInstanceLtsConfigs(const std::vector<InstanceLogConfig>& value)
{
