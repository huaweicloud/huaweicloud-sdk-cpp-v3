...
bool UpdateClientNetworkRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            ClientNetworkRequestBody refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
ClientNetworkRequestBody UpdateClientNetworkRequest::getBody() const
{
    return body_;
}

void UpdateClientNetworkRequest::setBody(const ClientNetworkRequestBody& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
