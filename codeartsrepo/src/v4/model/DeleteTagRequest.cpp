

#include "huaweicloud/codeartsrepo/v4/model/DeleteTagRequest.h"
namespace HuaweiCloud {
namespace Sdk {
namespace Codeartsrepo {
namespace V4 {
namespace Model {




DeleteTagRequest::DeleteTagRequest()
{
    repositoryId_ = 0;
    repositoryIdIsSet_ = false;
    tagName_ = "";
    tagNameIsSet_ = false;
}

DeleteTagRequest::~DeleteTagRequest() = default;

void DeleteTagRequest::validate()
{
}

web::json::value DeleteTagRequest::toJson() const
{
    web::json::value val = web::json::value::object();

    if(repositoryIdIsSet_) {
        val[utility::conversions::to_string_t("repository_id")] = ModelBase::toJson(repositoryId_);
    }
    if(tagNameIsSet_) {
        val[utility::conversions::to_string_t("tag_name")] = ModelBase::toJson(tagName_);
    }

    return val;
}
bool DeleteTagRequest::fromJson(const web::json::value& val)
{
    bool ok = true;
    
    if(val.has_field(utility::conversions::to_string_t("repository_id"))) {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t("repository_id"));
        if(!fieldValue.is_null())
        {
            int32_t refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setRepositoryId(refVal);
        }
    }
    if(val.has_field(utility::conversions::to_string_t("tag_name"))) {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t("tag_name"));
        if(!fieldValue.is_null())
        {
            std::string refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setTagName(refVal);
        }
    }
    return ok;
}


int32_t DeleteTagRequest::getRepositoryId() const
{
    return repositoryId_;
}

void DeleteTagRequest::setRepositoryId(int32_t value)
{
    repositoryId_ = value;
    repositoryIdIsSet_ = true;
}

bool DeleteTagRequest::repositoryIdIsSet() const
{
    return repositoryIdIsSet_;
}

void DeleteTagRequest::unsetrepositoryId()
{
    repositoryIdIsSet_ = false;
}

std::string DeleteTagRequest::getTagName() const
{
    return tagName_;
}

void DeleteTagRequest::setTagName(const std::string& value)
{
    tagName_ = value;
    tagNameIsSet_ = true;
}

bool DeleteTagRequest::tagNameIsSet() const
{
    return tagNameIsSet_;
}

void DeleteTagRequest::unsettagName()
{
    tagNameIsSet_ = false;
}

}
}
}
}
}


