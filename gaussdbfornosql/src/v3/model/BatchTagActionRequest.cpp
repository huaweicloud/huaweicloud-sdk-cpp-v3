...
bool BatchTagActionRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            BatchTagActionRequestBody refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
BatchTagActionRequestBody BatchTagActionRequest::getBody() const
{
    return body_;
}

void BatchTagActionRequest::setBody(const BatchTagActionRequestBody& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
