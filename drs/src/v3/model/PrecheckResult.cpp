...
bool PrecheckResult::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("failed_sub_jobs"))) {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<PrecheckFailSubJobVO> refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
...
std::vector<PrecheckFailSubJobVO>& PrecheckResult::getFailedSubJobs()
{
    return failedSubJobs_;
}

void PrecheckResult::setFailedSubJobs(const std::vector<PrecheckFailSubJobVO>& value)
{
