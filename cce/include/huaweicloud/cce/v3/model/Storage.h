...
/// <summary>
/// **参数解释**： 磁盘初始化配置管理参数。 该参数配置逻辑较为复杂，详细说明请参见[节点磁盘挂载](node_storage_example.xml)。 **约束限制**： - v1.15.11-r0及以上集群版本支持此字段。不支持集群版本填写此字段将被忽略。 - 如存在节点规格涉及本地盘并同时使用云硬盘场景时，请勿缺省此参数，避免出现将用户未期望的磁盘分区。 - 如希望数据盘取值范围调整至20-32768，请勿缺省此参数。 - 如希望使用共享磁盘空间(取消runtime和kubernetes分区)，请勿缺省此参数，共享磁盘空间请参考[数据盘空间分配说明](cce_01_0341.xml)。 - 如希望系统组件存储在系统盘中，请勿缺省此参数。  **取值范围**： 不涉及 **默认取值**： 不涉及 
/// </summary>
