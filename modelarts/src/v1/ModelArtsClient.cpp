...
std::shared_ptr<CreateInferTempApiKeyResponse> ModelArtsClient::createInferTempApiKey(CreateInferTempApiKeyRequest &request)
{
    std::string localVarPath = "/v2/{project_id}/services/api-keys/temp";
    ...
    std::unique_ptr<HttpResponse> res = callApi("POST", localVarPath, localVarPathParams, localVarQueryParams,
        localVarHeaderParams, localVarHttpBody, ModelArtsMeta::genRequestDefForCreateInferTempApiKey());

    std::shared_ptr<CreateInferTempApiKeyResponse> localVarResult = std::make_shared<CreateInferTempApiKeyResponse>();
    localVarResult->setStatusCode(res->getStatusCode());
    localVarResult->setHeaderParams(res->getHeaderParams());
    localVarResult->setHttpBody(res->getHttpBody());
    if (!res->getHttpBody().empty()) {
...
std::shared_ptr<ListInferServicesResponse> ModelArtsClient::listInferServices(ListInferServicesRequest &request)
{
    ...
    if (request.nodeIpIsSet()) {
        localVarQueryParams["node_ip"] = parameterToString(request.getNodeIp());
    }
    if (request.sortDirIsSet()) {
        localVarQueryParams["sort_dir"] = parameterToString(request.getSortDir());
    }
