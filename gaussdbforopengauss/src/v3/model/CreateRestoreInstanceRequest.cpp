...
bool CreateRestoreInstanceRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            OpengaussRestoreInstanceRequest refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
OpengaussRestoreInstanceRequest CreateRestoreInstanceRequest::getBody() const
{
    return body_;
}

void CreateRestoreInstanceRequest::setBody(const OpengaussRestoreInstanceRequest& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
