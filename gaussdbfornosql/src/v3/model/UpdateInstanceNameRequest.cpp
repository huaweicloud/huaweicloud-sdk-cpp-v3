...
bool UpdateInstanceNameRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            UpdateInstanceNameRequestBody refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
UpdateInstanceNameRequestBody UpdateInstanceNameRequest::getBody() const
{
    return body_;
}

void UpdateInstanceNameRequest::setBody(const UpdateInstanceNameRequestBody& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
