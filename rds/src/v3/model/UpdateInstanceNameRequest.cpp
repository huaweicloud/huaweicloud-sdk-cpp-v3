...
bool UpdateInstanceNameRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            ModifiyInstanceNameRequest refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
ModifiyInstanceNameRequest UpdateInstanceNameRequest::getBody() const
{
    return body_;
}

void UpdateInstanceNameRequest::setBody(const ModifiyInstanceNameRequest& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
