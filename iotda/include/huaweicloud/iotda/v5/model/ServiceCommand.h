
#ifndef HUAWEICLOUD_SDK_IOTDA_V5_MODEL_ServiceCommand_H_
#define HUAWEICLOUD_SDK_IOTDA_V5_MODEL_ServiceCommand_H_


#include <huaweicloud/iotda/v5/IoTDAExport.h>

#include <huaweicloud/core/utils/ModelBase.h>
#include <huaweicloud/core/utils/Utils.h>
#include <huaweicloud/core/http/HttpResponse.h>

#include <string>
#include <huaweicloud/iotda/v5/model/ServiceCommandPara.h>
#include <vector>
#include <huaweicloud/iotda/v5/model/ServiceCommandResponse.h>

namespace HuaweiCloud {
namespace Sdk {
namespace Iotda {
namespace V5 {
namespace Model {

using namespace HuaweiCloud::Sdk::Core::Utils;
using namespace HuaweiCloud::Sdk::Core::Http;
/// <summary>
/// 命令服务对象。
/// </summary>
class HUAWEICLOUD_IOTDA_V5_EXPORT  ServiceCommand
    : public ModelBase
{
public:
    ServiceCommand();
    virtual ~ServiceCommand();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;
    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;
    /////////////////////////////////////////////
    /// ServiceCommand members

    /// <summary>
    /// **参数说明**：设备命令名称。注：设备服务内不允许重复。 **取值范围**：长度不超过64，只允许中文、字母、数字、以及_?&#39;#().,&amp;%@!-等字符的组合。
    /// </summary>

    std::string getCommandName() const;
    bool commandNameIsSet() const;
    void unsetcommandName();
    void setCommandName(const std::string& value);

    /// <summary>
    /// **参数说明**：设备命令的参数列表。
    /// </summary>

    std::vector<ServiceCommandPara>& getParas();
    bool parasIsSet() const;
    void unsetparas();
    void setParas(const std::vector<ServiceCommandPara>& value);

    /// <summary>
    /// **参数说明**：设备命令的响应列表。
    /// </summary>

    std::vector<ServiceCommandResponse>& getResponses();
    bool responsesIsSet() const;
    void unsetresponses();
    void setResponses(const std::vector<ServiceCommandResponse>& value);


protected:
    std::string commandName_;
    bool commandNameIsSet_;
    std::vector<ServiceCommandPara> paras_;
    bool parasIsSet_;
    std::vector<ServiceCommandResponse> responses_;
    bool responsesIsSet_;

};


}
}
}
}
}

#endif // HUAWEICLOUD_SDK_IOTDA_V5_MODEL_ServiceCommand_H_
