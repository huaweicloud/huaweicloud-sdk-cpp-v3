

#include "huaweicloud/cfw/v1/model/ListDomainsResponse.h"
namespace HuaweiCloud {
namespace Sdk {
namespace Cfw {
namespace V1 {
namespace Model {




ListDomainsResponse::ListDomainsResponse()
{
    dataIsSet_ = false;
}

ListDomainsResponse::~ListDomainsResponse() = default;

void ListDomainsResponse::validate()
{
}

web::json::value ListDomainsResponse::toJson() const
{
    web::json::value val = web::json::value::object();

    if(dataIsSet_) {
        val[utility::conversions::to_string_t("data")] = ModelBase::toJson(data_);
    }

    return val;
}
bool ListDomainsResponse::fromJson(const web::json::value& val)
{
    bool ok = true;
    
    if(val.has_field(utility::conversions::to_string_t("data"))) {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t("data"));
        if(!fieldValue.is_null())
        {
            ListDomainResponseData refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setData(refVal);
        }
    }
    return ok;
}


ListDomainResponseData ListDomainsResponse::getData() const
{
    return data_;
}

void ListDomainsResponse::setData(const ListDomainResponseData& value)
{
    data_ = value;
    dataIsSet_ = true;
}

bool ListDomainsResponse::dataIsSet() const
{
    return dataIsSet_;
}

void ListDomainsResponse::unsetdata()
{
    dataIsSet_ = false;
}

}
}
}
}
}


