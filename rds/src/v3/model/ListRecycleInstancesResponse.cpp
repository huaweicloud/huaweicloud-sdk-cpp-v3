...
bool ListRecycleInstancesResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("instances"))) {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<RecycleInstsanceV3> refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setInstances(refVal);
        }
    }
...
std::vector<RecycleInstsanceV3>& ListRecycleInstancesResponse::getInstances()
{
    return instances_;
}

void ListRecycleInstancesResponse::setInstances(const std::vector<RecycleInstsanceV3>& value)
{
    instances_ = value;
