...
bool ListDbUsersResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("users"))) {
        ...
        {
            std::vector<RedisDbUserInfo> refVal;
...
std::vector<RedisDbUserInfo>& ListDbUsersResponse::getUsers()
{
    return users_;
}

void ListDbUsersResponse::setUsers(const std::vector<RedisDbUserInfo>& value)
{
    users_ = value;
    usersIsSet_ = true;
}
