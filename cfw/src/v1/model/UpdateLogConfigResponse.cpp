

#include "huaweicloud/cfw/v1/model/UpdateLogConfigResponse.h"
namespace HuaweiCloud {
namespace Sdk {
namespace Cfw {
namespace V1 {
namespace Model {




UpdateLogConfigResponse::UpdateLogConfigResponse()
{
    data_ = "";
    dataIsSet_ = false;
}

UpdateLogConfigResponse::~UpdateLogConfigResponse() = default;

void UpdateLogConfigResponse::validate()
{
}

web::json::value UpdateLogConfigResponse::toJson() const
{
    web::json::value val = web::json::value::object();

    if(dataIsSet_) {
        val[utility::conversions::to_string_t("data")] = ModelBase::toJson(data_);
    }

    return val;
}
bool UpdateLogConfigResponse::fromJson(const web::json::value& val)
{
    bool ok = true;
    
    if(val.has_field(utility::conversions::to_string_t("data"))) {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t("data"));
        if(!fieldValue.is_null())
        {
            std::string refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setData(refVal);
        }
    }
    return ok;
}


std::string UpdateLogConfigResponse::getData() const
{
    return data_;
}

void UpdateLogConfigResponse::setData(const std::string& value)
{
    data_ = value;
    dataIsSet_ = true;
}

bool UpdateLogConfigResponse::dataIsSet() const
{
    return dataIsSet_;
}

void UpdateLogConfigResponse::unsetdata()
{
    dataIsSet_ = false;
}

}
}
}
}
}


