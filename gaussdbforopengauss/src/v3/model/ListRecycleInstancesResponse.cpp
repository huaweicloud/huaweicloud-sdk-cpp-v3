...
bool ListRecycleInstancesResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("instances"))) {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<RecycleInstancesDetailResult> refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setInstances(refVal);
        }
    }
...
std::vector<RecycleInstancesDetailResult>& ListRecycleInstancesResponse::getInstances()
{
    return instances_;
}

void ListRecycleInstancesResponse::setInstances(const std::vector<RecycleInstancesDetailResult>& value)
{
    instances_ = value;
