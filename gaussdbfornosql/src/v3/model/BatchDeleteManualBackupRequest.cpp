...
bool BatchDeleteManualBackupRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            BatchDeleteManualBackupRequestBody refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
BatchDeleteManualBackupRequestBody BatchDeleteManualBackupRequest::getBody() const
{
    return body_;
}

void BatchDeleteManualBackupRequest::setBody(const BatchDeleteManualBackupRequestBody& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
