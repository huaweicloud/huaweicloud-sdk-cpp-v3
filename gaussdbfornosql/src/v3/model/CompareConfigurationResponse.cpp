...
bool CompareConfigurationResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("differences"))) {
        ...
        {
            std::vector<DifferentDetails> refVal;
...
std::vector<DifferentDetails>& CompareConfigurationResponse::getDifferences()
{
    return differences_;
}

void CompareConfigurationResponse::setDifferences(const std::vector<DifferentDetails>& value)
{
    differences_ = value;
