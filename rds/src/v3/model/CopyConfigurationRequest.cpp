...
bool CopyConfigurationRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            ConfigurationCopyRequestBody refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
ConfigurationCopyRequestBody CopyConfigurationRequest::getBody() const
{
    return body_;
}

void CopyConfigurationRequest::setBody(const ConfigurationCopyRequestBody& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
