...
bool CreateDatabaseRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            GaussDBforOpenDatabaseForCreation refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
GaussDBforOpenDatabaseForCreation CreateDatabaseRequest::getBody() const
{
    return body_;
}

void CreateDatabaseRequest::setBody(const GaussDBforOpenDatabaseForCreation& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
