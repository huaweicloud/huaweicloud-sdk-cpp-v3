...
bool CreateMergeRequestDiscussionRequest::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("body"))) {
        ...
        if(!fieldValue.is_null())
        {
            CreateMergeRequestNoteDto refVal;
            ok &= ModelBase::fromJson(fieldValue, refVal);
            setBody(refVal);
        }
    }
...
CreateMergeRequestNoteDto CreateMergeRequestDiscussionRequest::getBody() const
{
    return body_;
}

void CreateMergeRequestDiscussionRequest::setBody(const CreateMergeRequestNoteDto& value)
{
    body_ = value;
    bodyIsSet_ = true;
}
