...
bool ListSlowLogsResponse::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t("slow_log_list"))) {
        ...
        {
            std::vector<SlowlogResult> refVal;
...
std::vector<SlowlogResult>& ListSlowLogsResponse::getSlowLogList()
{
    return slowLogList_;
}

void ListSlowLogsResponse::setSlowLogList(const std::vector<SlowlogResult>& value)
{
    slowLogList_ = value;
